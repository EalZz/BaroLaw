...
try:
    from rag import LegalRAGPipeline, CORPUS_PATH, CACHE_PATH
    ...
    retriever = LegalRAGPipeline()
    retriever.initialize()
    
    elapsed = time.time() - start_time
    print(f"\n--- [v8.3.3 Warmup] Success! ---")
    print(f" - Size: {len(retriever.vector_retriever.embeddings)} items")

    print(f" - Time: {elapsed:.1f}s")
    print(f" - Device: {retriever.vector_retriever.model.device}")
